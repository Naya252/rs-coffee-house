...
</svg>`,x=`<svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg>`,B=f,O=w,E=m,j=v,S=k,A=L,H=b,R=M,Z=I,$=_,P=x,F=y,{BASE_URL:i}={BASE_URL:"https://rolling-scopes-school.github.io/naya252-JSFE2023Q4/coffee-house2/",MODE:"production",DEV:!1,PROD:!0,SSR:!1},r=[{name:"logo",link:"enjoy",anchor:"enjoy"},{name:"Favorite coffee",link:"favorite-coffee"},{name:"About",link:"about"},{name:"Mobile app",link:"mobile-app"},{name:"Contact us",link:"footer"},{name:"Menu",link:`${i}menu/`}];let c;const V="#",d=window.location.pathname;d==="/naya252-JSFE2023Q4/coffee-house2/menu/"?(c=`${i}#`,r[0].link=i):(c="#",r[0].link=`${i}#enjoy`);document.querySelector("#header").insertAdjacentHTML("afterbegin",`
...
`);const h=r.slice(1,-1);let C="";h.forEach(t=>{C+=`
  <li>
        <a
        id="nav-${t.link}"
          href="${t.link==="footer"?V+t.link:c+t.link}"
          class="nav-item">${t.name}</a>
  </li>
  `});document.querySelector("#nav-list").insertAdjacentHTML("afterbegin",C);function D(t){t.forEach(n=>{const l=n.target;n.isIntersecting?document.querySelectorAll(".nav-item").forEach(s=>{s.classList.toggle("active",s.getAttribute("id").replace("nav-","")===l.id)}):!n.isIntersecting&&d==="/naya252-JSFE2023Q4/coffee-house2/"&&document.querySelectorAll(".nav-item").forEach(s=>{s.classList.remove("active")})})}const u=new IntersectionObserver(D,{threshold:[.5,1]});d==="/naya252-JSFE2023Q4/coffee-house2/menu/"?(document.querySelector("#nav-menu").classList.add("active"),document.querySelector("#nav-menu").style.pointerEvents="none",["menu","footer"].forEach(t=>{u.observe(document.getElementById(t))})):h.forEach(t=>{u.observe(document.getElementById(t.link))});document.querySelector("#footer").insertAdjacentHTML("afterbegin",`
...
<div class="footer__contacts">
  ...
  <a
    ...
    ><span>${A} 8558 Green Rd., LA</span></a
  >
  <a
    class="link mb-1"
...
`);const N=[O,E,j];let g="";N.forEach(t=>{g+=`<button class="social__btn rounded-btn transparent-light-btn">${t}</button>`});document.querySelector("#socials").insertAdjacentHTML("afterbegin",g);export{R as A,i as B,B as C,P as G,F as R,Z as a,$ as b};
//# sourceMappingURL=main-e04d39ef.js.map
