...
`);let e="";pt.forEach(o=>{e+=`<button id="${o.name.toLowerCase()}">
  ...
  `}),document.querySelector(".menu__items").insertAdjacentHTML("afterbegin",t)}function gt(){u(),ft(),p()}function b(){document.querySelectorAll(".nav-item").forEach(e=>{e.classList.remove("active")})}function f(){r.includes("menu")&&(document.querySelector("#nav-menu").classList.add("active"),document.querySelector("#nav-menu").style.pointerEvents="none")}function ht(e){e.forEach(t=>{const o=t.target;t.isIntersecting?document.querySelectorAll(".nav-item").forEach(s=>{s.classList.toggle("active",s.getAttribute("id").replace("nav-","")===o.id)}):!t.isIntersecting&&!r.includes("menu")&&b()})}const g=new IntersectionObserver(ht,{threshold:[.5,1]});function h(){let e;return r.includes("menu")?(f(),e=["menu","footer"]):e=m.map(t=>t.link),e}function vt(){h().forEach(t=>{g.observe(document.getElementById(t))})}function At(){h().forEach(t=>{g.unobserve(document.getElementById(t))}),b(),f()}r.includes("menu")?gt():ut();const wt=new ResizeObserver(e=>{const[{inlineSize:t}]=e[0].borderBoxSize;t>1024?vt():At()}),Ct=document.querySelector("body");wt.observe(Ct);
//# sourceMappingURL=main-7d2c013e.js.map
