...
</svg>`,_=`<svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg>`,y=`<svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg>`,B=f,x=m,O=w,E=v,S=k,j=L,A=b,N=M,Z=I,$=_,P=y,{BASE_URL:i}={BASE_URL:"https://rolling-scopes-school.github.io/naya252-JSFE2023Q4/coffee-house/",MODE:"production",DEV:!1,PROD:!0,SSR:!1},r=[{name:"logo",link:"enjoy",anchor:"enjoy"},{name:"Favorite coffee",link:"favorite-coffee"},{name:"About",link:"about"},{name:"Mobile app",link:"mobile-app"},{name:"Contact us",link:"footer"},{name:"Menu",link:`${i}menu/`}];let c;const D="#",d=window.location.pathname;d==="/naya252-JSFE2023Q4/coffee-house/menu/"?(c=`${i}#`,r[0].link=i):(c="#",r[0].link=`${i}#enjoy`);document.querySelector("#header").insertAdjacentHTML("afterbegin",`
<div class="header__bar">
  <a href="${r[0].link}" id="logo"
    ><img
      src="${p}"
      alt="${r[0].name}"
  /></a>
...
`);const u=r.slice(1,-1);let C="";u.forEach(t=>{C+=`
  <li>
        <a
        id="nav-${t.link}"
          href="${t.link==="footer"?D+t.link:c+t.link}"
          class="nav-item">${t.name}</a>
  </li>
  `});document.querySelector("#nav-list").insertAdjacentHTML("afterbegin",C);function H(t){t.forEach(n=>{const l=n.target;n.isIntersecting?document.querySelectorAll(".nav-item").forEach(s=>{s.classList.toggle("active",s.getAttribute("id").replace("nav-","")===l.id)}):!n.isIntersecting&&d==="/naya252-JSFE2023Q4/coffee-house/"&&document.querySelectorAll(".nav-item").forEach(s=>{s.classList.remove("active")})})}const h=new IntersectionObserver(H,{threshold:[.5,1]});d==="/naya252-JSFE2023Q4/coffee-house/menu/"?(document.querySelector("#nav-menu").classList.add("active"),document.querySelector("#nav-menu").style.pointerEvents="none",["menu","footer"].forEach(t=>{h.observe(document.getElementById(t))})):u.forEach(t=>{h.observe(document.getElementById(t.link))});document.querySelector("#footer").insertAdjacentHTML("afterbegin",`
...
<div class="footer__contacts">
  ...
  <a
    ...
    ><span>${S} +1 (603) 555-0123</span></a
  >
  <span class="link mb-1">${A} Mon-Sat: 9:00 AM – 23:00 PM</span>
</div>
`);const V=[x,O,E];let g="";V.forEach(t=>{g+=`<button class="social__btn">${t}</button>`});document.querySelector("#socials").insertAdjacentHTML("afterbegin",g);export{N as A,i as B,B as C,P as G,Z as a,$ as b};
//# sourceMappingURL=main-789bd86a.js.map
